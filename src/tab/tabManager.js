...
class TabManager {
    ...
    #defineEvents() {
        ...
        this.on("beforeTabAdd", async (tab) => {
            ...
            if (tab.events.beforeTabAdd && typeof tab.events.beforeTabAdd === "function") {
                try {
                     // tab을 `this`로 참조
                    await tab.events.beforeTabAdd(tab); 
                } catch (error) {
                    console.error(`❌ Error occurred while executing beforeTabAdd: ${error.message}`);
                }
            }
        ...
        this.on("tabAdded", (tab) => {
            ...
            if (tab.events.tabAdded && typeof tab.events.tabAdded === "function") {
                try {
                     // tab을 `this`로 참조
                    tab.events.tabAdded(tab); 
                } catch (error) {
                    console.error(`❌ Error occurred while executing tabAdded: ${error.message}`);
                }
            }
        ...
        this.on("beforeTabRemove", async (tab) => {
            ...
            if (tab.events.beforeTabRemove && typeof tab.events.beforeTabRemove === "function") {
                try {
                    // `tab`을 `this`로 참조하여 호출
                    await tab.events.beforeTabRemove(tab);
                } catch (error) {
                    console.error(`❌ Error occurred while executing beforeTabRemove: ${error.message}`);
                }
            }
        
            return true; // 기본적으로 이벤트 허용
        });
        ...
        this.on("tabRemoved", (tab) => {
            ...
            if (tab.events.tabRemoved && typeof tab.events.tabRemoved === "function") {
                try {
                    // `tab`을 `this`로 참조하여 호출
                    tab.events.tabRemoved(tab);
                } catch (error) {
                    console.error(`❌ Error occurred while executing tabRemoved: ${error.message}`);
                }
            }
        });
        ...
        this.on("tabClicked", (tab) => {
            ...
            if (tab.events.tabClicked && typeof tab.events.tabClicked === "function") {
                try {
                    // `tab`을 `this`로 참조하여 호출
                    tab.events.tabClicked(tab);
                } catch (error) {
                    console.error(`❌ Error occurred while executing tabClicked: ${error.message}`);
                }
            }
        });
        ...
        this.on("beforeTabDeactivate", async ({ prevTab, newTab }) => {
            ...
            if (prevTab.events.beforeTabDeactivate && typeof prevTab.events.beforeTabDeactivate === "function") {
                try {
                    // `tab`을 `this`로 참조하여 호출
                    await prevTab.events.beforeTabDeactivate({prevTab, newTab});
                } catch (error) {
                    console.error(`❌ Error occurred while executing beforeTabDeactivate: ${error.message}`);
                }
            }
        ...
        this.on("beforeTabActivate", async ({ prevTab, newTab }) => {
            ...
            if (newTab.events.beforeTabActivate && typeof newTab.events.beforeTabActivate === "function") {
                try {
                    // `tab`을 `this`로 참조하여 호출
                    await newTab.events.beforeTabActivate({ prevTab, newTab });
                } catch (error) {
                    console.error(`❌ Error occurred while executing beforeTabActivate: ${error.message}`);
                }
            }
        ...
        this.on("tabDeactivated", ({ prevTab, newTab }) => {
            ...
            if (prevTab.events.tabDeactivated && typeof prevTab.events.tabDeactivated === "function") {
                try {
                    // `tab`을 `this`로 참조하여 호출
                    prevTab.events.tabDeactivated({ prevTab, newTab });
                } catch (error) {
                    console.error(`❌ Error occurred while executing tabDeactivated: ${error.message}`);
                }
            }
        });
        ...
        this.on("tabActivated", ({ prevTab, newTab }) => {
            ...
            if (newTab.events.tabActivated && typeof newTab.events.tabActivated === "function") {
                try {
                    // `tab`을 `this`로 참조하여 호출
                    newTab.events.tabActivated({ prevTab, newTab });
                } catch (error) {
                    console.error(`❌ Error occurred while executing tabActivated: ${error.message}`);
                }
            }
        });
    ...
    setMaxTabSize(maxTabSize){

        // number가 아닌 경우 예외처리
        if(typeof maxTabSize !== "number"){
            console.error(`❌ [%cTabManager%c] Invalid maxTabSize: %c${maxTabSize} (Number required)`);
            return;
        }
    ...
    setOptions(options = {}) {
        ...
        // number가 아닌 경우 예외처리
        if(options.maxTabSize && typeof options.maxTabSize == "number"){
            this.maxTabSize = options.maxTabSize
        }
    }
    ...
    async redraw(isInit = false) {
        ...
        // 초기화 전, 각 tab의 innerHTML을 저장
        for (const tab of this.state.tabs) {
            let tab_content = this.getLoadedTabContent(tab.tab_id);

            temp_tabContent[tab.tab_id] = tab_content;
        }
        ...
        for (const tab of this.state.tabs) {
            await this.addTab({
                ...tab,
                tab_content: temp_tabContent[tab.tab_id]
            }, false); // ✅ `emit` 실행 방지

            // 메모리클리어 (함수 종료 전에 클리어)
            temp_tabContent[tab.tab_id] = undefined;
        }

        let tobeActiveTab = this.state.activeTab;
        if(tobeActiveTab){
            this.setActiveTab(tobeActiveTab);
        }
    ...
    getLoadedTabContent(tabId) {

        const tabContentElem = document.querySelector(`.tab-content[data-tab-id="${tabId}"]`);

        return tabContentElem ? tabContentElem.innerHTML : undefined;
    }
    ...
    async addTab({ 
            ...
            load_model = false, icon = '', isRemovable = true ,
            events = {},
        }, emitEvent = true) {
        ...
        ///////////////////////
        // events callback 클로져 바인딩

        /*
        const eventNames = [
            "beforeTabAdd", "tabAdded", "beforeTabRemove", "tabRemoved", 
            "tabClicked", "beforeTabDeactivate", "beforeTabActivate", 
            "tabDeactivated", "tabActivated"
        ];
        
        eventNames.forEach(eventName => {
            if (events[eventName]) {
                const evtMethod = events[eventName];
                events[eventName] = function(...args) {
                    // 원본 함수 호출, 인자는 그대로 전달
                    return evtMethod.apply(this, args);
                };
            }
        });
        */
        ...
        let tabObj = { tab_id, tab_title, icon, events, tabData: {} };
        ...
        if(emitEvent){
            ...
            // maxTabSize 체크
            if(this.maxTabSize <= 0){
                // 무제한
                this.#log(`✅ [%cTabManager%c] maximum tab size is Unlimited`, 
                    "color: #673AB7; font-weight: bold;", "", 
                    "color: #4CAF50; font-weight: bold;");

            }else if (this.state.tabs.length >= this.maxTabSize && this.maxTabSize !== 0) {
                console.error(`❌ [%cTabManager%c] Maximum tab size reached: %c${this.maxTabSize}`,
                    "color: #673AB7; font-weight: bold;", "",
                    "color: #FF0000; font-weight: bold;"
                );
                return;
